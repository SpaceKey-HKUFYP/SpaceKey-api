...
public class BEntry {
	...
	public BEntry(double xl, double xh, double yl, double yh){
		x = new Pair();
		y = new Pair();
		x.l = xl;
		x.h = xh;
	...
	public double diffArea(BEntry e){
		return child.diffArea(e);
	}
	public double overlap(BEntry e){
		double xl, xh, yl, yh;
	...
	public static Comparator<BEntry> CompareX = new Comparator<BEntry>() {
		public int compare(BEntry e1, BEntry e2) {
			if (e1.x.l == e2.x.l && e1.x.h != e2.x.h) return (e1.x.h - e2.x.h > 0? 1 : -1);
			else if (e1.x.l != e2.x.l) return (e1.x.l - e2.x.l > 0? 1: -1);
			else return 0;
		}
	};
	public static Comparator<BEntry> CompareY = new Comparator<BEntry>() {
		public int compare(BEntry e1, BEntry e2) {
	...
	public static Comparator<BEntry> CompareDist = new Comparator<BEntry>() {
		public int compare(BEntry e1, BEntry e2) {
